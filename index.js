  ...
  while (true) {
    ...
    screens[SCREEN_PERF] = screen
      .map(([header, percent], index) => {
        ...
        )}${" ".repeat(barGraphSize - numBlackTiles)}|${title(index, 0)}`;
      })
      .join("");
...
async function startStockMonitor() {
  ...
  while (true) {
    ...
    for (const [key, value] of stocks) {
      ...
      lines.push(
        `${line}${" ".repeat(16 - line.length)}|  ${title(lines.length, 1)} `
      );
    }
...
async function startWeatherMonitor() {
  ...
  function getWeather() {
    return new Promise((resolve) => {
      request(
        `https://www.yahoo.com/news/weather/united-states/st-augustine/st-augustine-12771497`,
        (err, res, body) => {
          const weather = {};
